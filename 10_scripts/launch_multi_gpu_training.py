...
def check_gpu_availability():
    ...
    if not torch.cuda.is_available():
        print("❌ CUDA not available - will use CPU-only training")
        return 0, "cpu"
...
def setup_wandb():
    ...
    if not wandb_key:
        print("⚠️  WandB API key not found. Set WANDB_API_KEY environment variable.")
...
def main():
    ...
    if not args.skip_wandb:
        ...
        if not wandb_available:
            print("⚠️  Continuing without WandB logging...")
    else:
